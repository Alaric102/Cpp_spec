...
class RouteManager {
public:
  void AddRoute(int start, int finish) { // 2*log N
    reachable_lists_[start].push_back(finish);  // log N
    reachable_lists_[finish].push_back(start);  // log N
  }
  int FindNearestFinish(int start, int finish) const {
    ...
    const vector<int>& reachable_stations = reachable_lists_.at(start); // log N
