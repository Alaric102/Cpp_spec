...
public:
  ...
  AirportCounter(TIterator begin, TIterator end){
    ...
    for (auto i = 0; i < _data.size(); ++i){
      std::get<TAirport>(_data[i]) = static_cast<TAirport>(i);
    }
    for (auto it = begin; it < end; ++it){
      std::get<TAirport>(_data[static_cast<size_t>(*it)]) = *it;
      std::get<size_t>(_data[static_cast<size_t>(*it)])++;
    }
  };

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const{
    return std::get<size_t>(_data[static_cast<size_t>(airport)]);
  };
  // добавить данный элемент
  void Insert(TAirport airport){
    std::get<size_t>(_data[static_cast<size_t>(airport)])++;
  };
  ...
  void EraseAll(TAirport airport){
    std::get<size_t>(_data[static_cast<size_t>(airport)]) = 0;
  };
